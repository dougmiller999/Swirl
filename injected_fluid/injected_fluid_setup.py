...
- injection velocity 100 rpm (Ω rad/s) = 10 pi
...
if run_label == "fast":
    # this piles up liquid pretty hard
    rpm = 100.0
    Omega = rpm * 2.0*np.pi / 60.0   # rad/s
    ut_inject = Omega * R # m/s
    ...
    raise RuntimeError("unknown value for 'run_label' variable")
...
max_iter = 4000          # SIMPLE iterations
...
if graphics:
    ...
    plot_pressure_contours(p, grid,
                           ...
                           z_min=wall_zmin, z_max=wall_zmax, showWall=True)
